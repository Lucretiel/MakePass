...
The most obvious security hole I'm currently aware of in ``makepass`` is that it writes to stdout, which means that it writes your terminal if you run it without a pipeline or redirect, which may be logged or cached to disk. Make sure to pipe it into a secure destintion when creating a password you actually intend to use; I'm personally partial to the ``say`` command on OSX, which speaks the password out loud through your speakers.
